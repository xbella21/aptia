...
export default function Ranking() {
  ...
  const alignStyle = a => ({ alta: { background: '#e8f5e3', color: '#2d5a27' }, parcial: { background: '#fef9e7', color: '#856404' }, sin_coincidencia: { background: '#f8f9fa', color: '#6b6455' } }[a] || {});
      ...
      <div style={styles.container}>
        ...
        {error && (
          <div style={styles.error}>
            {error.includes('confirmar') ? 'Debes confirmar tus puntajes ICFES primero.' : error}
            {error.includes('confirmar') && <span style={styles.errorLink} onClick={() => navigate('/icfes')}> Ir a ICFES</span>}
          </div>
        )}
        {loading ? (
            ...
            {ranking.map((item, i) => (
              <div key={i} style={styles.row}>
                ...
                <div style={styles.info}>
                  ...
                  <span style={{ ...styles.badge, ...alignStyle(item.alignmentLevel) }}>{alignLabel(item.alignmentLevel)}</span>
                </div>
                <span style={styles.pts}>{item.weightedScore} pts</span>
              </div>
            ))}
            {ranking.length === 0 && !error && <p style={{ color: 'var(--text-mid)', textAlign: 'center', padding: '40px 0' }}>Sin datos aun.</p>}
          </div>
        )}
      </div>
    </div>
