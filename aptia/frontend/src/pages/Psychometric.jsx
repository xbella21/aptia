...
export default function Psychometric() {
  ...
  const submit = async () => {
    ...
    setError('');
    try {
      await api.post('/psychometric/submit', { answers: Object.values(answers) });
      setSuccess(true);
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.container}>
        <h2 style={styles.title}>Prueba psicometrica</h2>
        <p style={styles.sub}>Responde con honestidad segun tu interes en cada area</p>
        ...
        {loading ? (
          ...
          <>
              ...
              return (
                <div key={q._id} style={styles.questionCard}>
                  ...
                  <div style={styles.options}>
                    {[1,2,3,4,5].map(val => (
                      <button key={val} style={{ ...styles.optionBtn, ...(selected === val ? styles.optionActive : {}) }} onClick={() => handleAnswer(q, val)}>
            ...
            {questions.length > 0 && (
              <button style={{ ...styles.btn, opacity: (submitting || answered < questions.length) ? 0.6 : 1 }} onClick={submit} disabled={submitting || answered < questions.length}>
...
const styles = {
  ...
  optionBtn: { flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', padding: '10px 4px', background: 'var(--beige)', border: '1.5px solid transparent', borderRadius: '8px', cursor: 'pointer' },
  optionActive: { background: 'var(--green-pale)', border: '1.5px solid var(--green-light)' },
