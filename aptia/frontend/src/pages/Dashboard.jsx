...
export default function Dashboard() {
  ...
  useEffect(() => {
    api.get('/icfes/me').then(({ data }) => setIcfes(data.data)).catch(() => {});
  }, []);

  const icfesPercent = icfes ? Math.min((icfes.scoreGlobal / 500) * 100, 100) : 0;

  const menu = [
    { label: 'Registrar puntaje ICFES', path: '/icfes', badge: null, icon: 'P' },
    { label: 'Prueba psicometrica', path: '/psychometric', badge: 'Nuevo', icon: 'T' },
    { label: 'Ver ranking', path: '/ranking', badge: null, icon: 'R' },
  ];
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.hero}>
        ...
        <p style={styles.heroDesc}>Aqui esta tu progreso en APTIA</p>
      </div>
      <div style={styles.container}>
        <div style={styles.cards}>
          <div style={styles.card}>
            <div style={styles.cardTop}><span style={styles.cardLabel}>Puntaje ICFES</span></div>
            <div style={styles.cardVal}>{icfes ? icfes.scoreGlobal : '-'}</div>
            <div style={styles.progressBar}><div style={{ ...styles.progressFill, width: icfesPercent + '%' }} /></div>
            <p style={styles.cardHint}>{icfes ? Math.round(icfesPercent) + '% del maximo' : 'Sin registrar'}</p>
          </div>
          <div style={styles.card}>
            <div style={styles.cardTop}><span style={styles.cardLabel}>Perfil vocacional</span></div>
...
const styles = {
  ...
  card: { background: 'var(--white)', border: '0.5px solid var(--beige-border)', borderRadius: '16px', padding: '18px' },
  ...
  progressFill: { height: '100%', background: 'var(--green-dark)', borderRadius: '2px' },
  ...
  menuItem: { display: 'flex', alignItems: 'center', gap: '12px', padding: '16px', background: 'var(--white)', border: '0.5px solid var(--beige-border)', borderRadius: '14px', cursor: 'pointer' },
  menuIcon: { fontSize: '14px', width: '32px', height: '32px', background: 'var(--green-pale)', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--green-dark)', fontWeight: '700' },
